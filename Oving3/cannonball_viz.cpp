...
void cannonBallViz(double targetPosition, int fieldLength, double initVelocityX,
                   double initVelocityY, int timeSteps) {
    ...
    for (int i = 0; i <= timeSteps; ++i) {
        // Lag en ny frame ved starten av løkken slik at siste bildet er vist
        // naar animasjonen er ferdig
        window.next_frame();

        // Plot target. targetPosition er [100, 1000] i main programmet
        // denne funksjonen mapper 0 til 0, og fieldLength til w_width
        int target_x =
            static_cast<int>((targetPosition * w_width) / fieldLength);
        ...
        int xPos = static_cast<int>(cannonball::posX(0, initVelocityX, time) *
                                    w_width) /
                   fieldLength;
        window.draw_circle(
            Point{xPos,
                  static_cast<int>(w_height -
                                   cannonball::posY(0, initVelocityY, time))},
            8, Color::red);
    }
